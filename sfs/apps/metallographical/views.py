...
from metallographical import models
...
class MetShow(View):
    ...
    def get(self,request):
        met_queryset = models.Metal.objects.defer("version", "create_time", "update_time").filter(
            is_delete=False)
        total_data_num = len(met_queryset)
        # 分页处理
        # 1.判断页码格式
        try:
        ...
        page_obj = Paginator(met_queryset, constant.PER_PAGE_NUMBER)
        # 2.判断页码是否为空

        try:
            met_info = page_obj.page(page_num)
        except Exception as e:
            logger.info("当前页数据获取异常:{}".format(e))
            met_info = page_obj.page(page_obj.num_pages)

        pages_data = per_page.get_page_data(page_obj, met_info)
        ...
        return render(request, "admin/metallographical/met_index.html", context=data)
